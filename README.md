...
- `Enabled Image Formats` lets you choose which image formats the indexer processes; file watchers only monitor the selected formats. This fine-grained control is especially helpful when using an entire volume as an image source but you want to selectively include or exclude specific formats. Native formats (PNG, JPG/JPEG, HEIC, BMP, GIF, HDR, PIC), other macOS formats (HEICS, HEIF, PICT, TIFF, WebP), and RAW formats (DNG, RAW) are enabled by default.
...
💡 All settings in this section are saved per query. `New query` resets them to the default values, except **Max results**, which carries over from your last query. To store and re-use your personal default settings, simply copy or rename your favourite queries in your project and click on it to restore all its settings instead of clicking `New query`.
